...
#define Z80_READ_BYTE(address, x)	(x)=mem_remap_rd((address));

#define Z80_FETCH_BYTE(address, x)	Z80_READ_BYTE((address), (x))

#define Z80_READ_WORD(address, x)	(x)=(mem_remap_rd((address)+0)|(mem_remap_rd((address)+1)<<8));

#define Z80_FETCH_WORD(address, x)	Z80_READ_WORD((address), (x))

#define Z80_WRITE_BYTE(address, x)	mem_remap_wr((address),(x));

#define Z80_WRITE_WORD(address, x)	mem_remap_wr((address+0),(unsigned char)(x)); mem_remap_wr((address+1),(unsigned char)((x)>>8));
...
#define Z80_INPUT_BYTE(port, x)		(x)=0xff;

#define Z80_OUTPUT_BYTE(port, x)	if(port_out((port),(x),elapsed_cycles)) { elapsed_cycles = number_cycles; goto stop_emulation; }
