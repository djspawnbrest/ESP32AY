...
int I2SClass::_installDriver(){
  ...
  if(_mode == ADC_DAC_MODE){
    ...
    if(adc_unit == esp_i2s::ADC_UNIT_1){
      ...
      esp_i2s::adc1_config_channel_atten((esp_i2s::adc1_channel_t)adc_channel, esp_i2s::ADC_ATTEN_DB_12);
    }else if(adc_unit == esp_i2s::ADC_UNIT_2){
      esp_i2s::adc2_config_channel_atten((esp_i2s::adc2_channel_t)adc_channel, esp_i2s::ADC_ATTEN_DB_12);
    }
