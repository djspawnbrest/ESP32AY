...
static uint16_t volSum[96]={0};
static uint8_t noteCount[96]={0};
static uint8_t channelCount[8]={0};
static bool eq_row_processed=false;
...
static float IRAM_ATTR xm_waveform(xm_waveform_type_t waveform, uint8_t step)
{
...
static void IRAM_ATTR xm_autovibrato(xm_context_t* ctx, xm_channel_context_t* ch)
{
...
static void IRAM_ATTR xm_vibrato(xm_context_t* ctx, xm_channel_context_t* ch, uint8_t param, uint16_t pos)
{
...
static void IRAM_ATTR xm_tremolo(xm_context_t* ctx, xm_channel_context_t* ch, uint8_t param, uint16_t pos)
{
  unsigned int step = pos * (param >> 4);
...
static void IRAM_ATTR xm_arpeggio(xm_context_t* ctx, xm_channel_context_t* ch, uint8_t param, uint16_t tick)
{
  switch (tick % 3)
  {
    case 0:
...
static void IRAM_ATTR xm_tone_portamento(xm_context_t* ctx, xm_channel_context_t* ch)
{
...
static void IRAM_ATTR xm_pitch_slide(xm_context_t* ctx, xm_channel_context_t* ch, float period_offset)
{
...
static void IRAM_ATTR xm_panning_slide(xm_channel_context_t* ch, uint8_t rawval)
{
  float f;
...
static void IRAM_ATTR xm_volume_slide(xm_channel_context_t* ch, uint8_t rawval)
{
  float f;
...
static float IRAM_ATTR xm_envelope_lerp(xm_envelope_point_t* a, xm_envelope_point_t* b, uint16_t pos)
{
...
static float IRAM_ATTR xm_linear_frequency(float period)
{
  return 8363.f * powf(2.f, (4608.f - period) / 768.f);
}

static float IRAM_ATTR xm_amiga_period(float note)
{
...
static float IRAM_ATTR xm_amiga_frequency(float period)
{
  if (period == .0f)
    return .0f;
...
static float IRAM_ATTR xm_period(xm_context_t* ctx, float note)
{
...
static float IRAM_ATTR xm_frequency(xm_context_t* ctx, float period, float note_offset)
{
  uint8_t  a;
  int8_t   octave;
  float    note;
...
static void IRAM_ATTR xm_update_frequency(xm_context_t* ctx, xm_channel_context_t* ch)
{
...
static void IRAM_ATTR xm_handle_note_and_instrument(xm_context_t* ctx, xm_channel_context_t* ch, xm_pattern_slot_t* s)
{
  if (s->instrument > 0)
  {
...
static void IRAM_ATTR xm_trigger_note(xm_context_t* ctx, xm_channel_context_t* ch, unsigned int flags)
{
...
static void IRAM_ATTR xm_cut_note(xm_channel_context_t* ch)
{
  /* NB: this is not the same as Key Off */
  ch->volume = .0f;
}

static void IRAM_ATTR xm_key_off(xm_channel_context_t* ch)
{
  /* Key Off */
  ch->sustained = false;
...
static void IRAM_ATTR xm_envelope_tick(xm_channel_context_t* ch, xm_envelope_t* env, uint16_t* counter, float* outval)
{
  if (env->num_points < 2)
  {
...
static void IRAM_ATTR xm_envelopes(xm_channel_context_t* ch)
{
  if (ch->instrument != NULL)
  {
...
static void IRAM_ATTR xm_tick(xm_context_t* ctx)
{
  ...
  for (uint8_t i = 0; i < ctx->module.num_channels; ++i)
  {
    ...
    switch (ch->current->effect_type)
    {
      ...
      case 17: /* Hxy: Global volume slide */
        ...
        if ((ch->global_volume_slide_param & 0xF0) && (ch->global_volume_slide_param & 0x0F))  /* Illegal state */
          break;
      ...
      case 27: /* Rxy: Multi retrig note */
        ...
        if (((ch->multi_retrig_param) & 0x0F) == 0)
          break;
  ...
  switch (ch->sample->loop_type)
  {
    ...
    case XM_FORWARD_LOOP:
      ...
      if (ch->sample->loop_length > 0) {
        ...
        // If loop_length is 0, clamp to loop_end
        if (ch->sample_position >= ch->sample->loop_end) {
          ch->sample_position = ch->sample->loop_end - 1;
        }
      }
...
bool IRAM_ATTR xm_sample(xm_context_t* ctx, float* left, float* right)
{
  ...
  *right = 0.f;
  ...
  if(ctx->max_loop_count==0&&ctx->current_table_index==ctx->module.restart_position&&ctx->current_row==0&&ctx->current_tick==0){
  ...
  for (uint8_t i = 0; i < ctx->module.num_channels; ++i)
  {
    ...
    if (!ch->muted && !ch->instrument->muted)
    {
      ...
      *right+=fval*ch->actual_volume*mod_panning;
    }
