...
int xm_create_context_safe(xm_context_t** ctxp, void* moddata, size_t moddata_length, uint32_t rate, MFUNC mfunc)
{
  ...
  mempool = mfunc(bytes_needed);
	...
	ctx->amplification = 1.f; /* Increased from 0.25f to 1.5f to compensate for typical XM file volumes (instruments often at 50-75%) */
...
const char* xm_get_instrument_name(xm_context_t* ctx,uint16_t instr){
	return ctx->module.instruments[instr-1].name;
}
#else
const char* xm_get_module_name(xm_context_t* ctx){
	return NULL;
